...
constexpr size_t BLOCK_SIZE = 16;
    ...
    Mode::CBC<BLOCK_SIZE>::encrypt(iv, subject, [](unsigned char*) {
      // encrypt the block here.
    });

    t.byte_eq(subject, validator, sizeof(validator), "CBC Mode Block 16 Encrypt");

    Mode::CBC<BLOCK_SIZE>::decrypt(iv_bkcup, subject, [](unsigned char*) {
...
constexpr size_t BLOCK_SIZE = 15;
    unsigned char iv_original[BLOCK_SIZE] = {
      0xde, 0xad, 0xbe, 0xef, 0xca, 0xfe, 0xba, 0xbe,
      0xba, 0xd2, 0xde, 0xed, 0xff, 0x11, 0x22,
    };
    ...
    unsigned char subject[BLOCK_SIZE] = {
      ...
      0xff, 0x22, 0x22, 0x22, 0x22, 0x22, 0xdd
    };
...
constexpr size_t BLOCK_SIZE = 17;
    unsigned char iv_original[BLOCK_SIZE] = {
      0xde, 0xad, 0xbe, 0xef, 0xca, 0xfe, 0xba, 0xbe,
      0xba, 0xd2, 0xde, 0xed, 0xff, 0x11, 0x22, 0xcc,
      0xf1
    };
    ...
    unsigned char subject[BLOCK_SIZE] = {
      0x00, 0x01, 0x02, 0x03, 0xff, 0xff, 0xff, 0xff,
      0xff, 0x22, 0x22, 0x22, 0x22, 0x22, 0xdd, 0xfa,
      0x0b
    };
  ...
  std::cout << "CBC Mode Test two 8 Block : \n";
  {
    ...
    for (size_t i = 0; i < BLOCKS; i++) {
      Mode::CBC<BLOCK_SIZE>::encrypt(iv, &subject[i * BLOCK_SIZE], [](unsigned char*) {
        // encrypt the block here.
      });
    }
    ...
    for (size_t i = 0; i < BLOCKS; i++) {
      Mode::CBC<BLOCK_SIZE>::decrypt(iv_bkcup, &subject[i * BLOCK_SIZE], [](unsigned char*) {
        // decrypt the block here.
      });
    }
