...
namespace smlts {
  ...
  class test {
    ...
    void byte_neq(unsigned char *a, unsigned char *b, size_t numBytes, std::string const &test_name);
  ...
  void test::byte_neq(unsigned char *a, unsigned char *b, size_t numBytes, std::string const &test_name) {
    test_number++;
    int result = !memcmp(a, b, numBytes);
    test_results.push_back(result);
    final_verdict |= result;
